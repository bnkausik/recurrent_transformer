...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            wpe = nn.Embedding(config.block_size, config.n_embd) if self.config.pos_flag else None,
    ...
    def forward(self, idx, targets=None, w = 1):
        ...
        if self.config.pos_flag:
            pos = torch.arange(0,t, dtype=torch.long, device=device)
            pos_emb = self.transformer.wpe(pos) # position embeddings
            x = self.transformer.drop(tok_emb + pos_emb)
        else:
            x = self.transformer.drop(tok_emb) # no positional embedding

        if not self.config.rec_flag:  #for regular transformer
            ...
            for block in self.transformer.h:
                ...
                while True:
                    if i+w <=  t:
                        #handle full block
                        #remember or forget oldest state
                        if self.config.mem_flag: x1 = torch.cat((0.5*x1[:,0:w]+x1[:,w:],0.5*x1[:,0:w]+x[:,i:i+w]),dim=1)
                        else: x1 = torch.cat((x1[:,w:],x[:,i:i+w]),dim=1)
                    else:
                        #handle remnant block
                        #remember or forget oldest state
                        if self.config.mem_flag: x1 = torch.cat((0.5*x1[:,0:w]+x1[:,w:],0.5*x1[:,0:t-i]+x[:,i:]),dim=1)
                        else: x1 = torch.cat((x1[:,w:],x[:,i:]),dim=1)
                    x1 = block(x1)
                    if i == w:
