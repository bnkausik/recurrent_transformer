...
def train():
    ...
    for step in range(num_steps):
        ...
        w = np.random.randint(gptconfig.w0,gptconfig.w1 + 1)
        with ctx:
            outputs,loss = model(inputs,targets,w)   #randomized block
        ...
        if step and ((not step % log_interval) or (step == num_steps-1)):
            ...
            print(f'{step+1} {w} Samples: {step*batch_size} train_loss: {mean_loss:.3f} sem_loss: {sem_loss:.3e} train_acc: {mean_acc:.2f} sem_acc: {sem_acc:.3e}  lr: {cur_lr:.2e}', end=' ')
            validate()
            model.train()
...
def validate():
    ...
    for step in range(log_interval):
        ...
        with ctx:
            outputs,loss = model(inputs,targets,int(0.5*(gptconfig.w0+gptconfig.w1)))
